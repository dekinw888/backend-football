...
exports.placeBetsBatch = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const matchRes = await client.query(
        "SELECT id, status, kickoff_time FROM matches WHERE id = $1 FOR UPDATE",
        [item.match_id]
      );
      const match = matchRes.rows[0];
      ...
      const oddsRes = await client.query('SELECT * FROM odds WHERE match_id = $1', [item.match_id]);
      const odds = oddsRes.rows[0];
      if (!odds) {
        await client.query('ROLLBACK');
...
exports.placeTicket = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const odds = (await client.query('SELECT * FROM odds WHERE match_id = $1', [item.match_id])).rows[0];
      const oddsRate = {
        HOME: odds && odds.odds_home,
        AWAY: odds && odds.odds_away,
        OVER: odds && odds.odds_over,
        UNDER: odds && odds.odds_under
      }[item.bet_selection];
