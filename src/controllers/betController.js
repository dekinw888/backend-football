...
exports.placeBet = async (req, res) => {
  ...
  try {
    ...
    // ฝั่งไหนเป็นทีมต่อจริงๆ ณ ตอนแทง (hdp ติดลบ = ทีมต่อ) เก็บไว้กันสลับ
    // ต่อ/รอง ผิดตอนแสดงผลย้อนหลัง เพราะราคาต่อรองอาจถูกแก้ทีหลังได้
    const favSide = Number(odds.hdp_home) < 0 ? 'HOME' : Number(odds.hdp_away) < 0 ? 'AWAY' : null;
    ...
    const newBet = await client.query(
      `INSERT INTO bets (user_id, match_id, bet_type, bet_selection, stake, odds_rate, fav_side)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [userId, match_id, bet_type, bet_selection, numericStake, oddsRate, favSide]
    );
  ...
  try {
    ...
    for (const item of items) {
      ...
      const betRes = await client.query(
        `INSERT INTO bets (user_id, match_id, bet_type, bet_selection, stake, odds_rate, fav_side)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [userId, item.match_id, item.bet_type, item.bet_selection, Number(item.stake), oddsRate, favSide]
      );
...
exports.placeTicket = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const favSide = Number(odds.hdp_home) < 0 ? 'HOME' : Number(odds.hdp_away) < 0 ? 'AWAY' : null;
      await client.query(
        `INSERT INTO bet_ticket_items (ticket_id, match_id, bet_type, bet_selection, odds_rate, fav_side)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [ticket.id, item.match_id, item.bet_type, item.bet_selection, oddsRate, favSide]
      );
    }
  ...
  try {
    ...
    const tickets = await db.query(
      `SELECT t.*, json_agg(json_build_object(
        ...
        'odds_rate', i.odds_rate, 'result', i.result, 'fav_side', i.fav_side,
