...
exports.getAllMatches = async (req, res) => {
  try {
    const query = `
      SELECT m.id, m.home_team, m.away_team, m.kickoff_time, m.status, m.league,
...
exports.createMatch = async (req, res) => {
  const { home_team, away_team, kickoff_time, league, hdp_home, hdp_away, over_under, odds_home, odds_away, odds_over, odds_under } = req.body;
  ...
  try {
    ...
    const matchResult = await db.query(
      'INSERT INTO matches (home_team, away_team, kickoff_time, league) VALUES ($1, $2, $3, $4) RETURNING *',
      [home_team, away_team, kickoff_time, league || '']
    );
...
exports.updateMatch = async (req, res) => {
  ...
  try {
    ...
    const matchResult = await client.query(
      `UPDATE matches SET home_team = $1, away_team = $2, kickoff_time = $3, league = $4
       WHERE id = $5 AND status = 'OPEN' RETURNING *`,
      [home_team, away_team, kickoff_time, league || '', matchId]
    );
